...
export default {
  ...
  fetchSchema(xmlPath, schemaLocation, callback) {
    ...
    if (_.isNil(schemaUrl.protocol) || _.isEmpty(schemaUrl.protocol) || schemaUrl.protocol === 'file:') {
      ...
      schemaPath = path.resolve(path.dirname(xmlPath), schemaPath);

      // Check if file exists and parse
      fs.exists(schemaPath, ((xsdFileExists) => {
        if (xsdFileExists) {
          fs.readFile(schemaPath, (err, rawData) => {
            if (err) {
              this.writeToPanel(err.message);
              return callback(err);
            } else {
              try {
                let schema = libxmljs.parseXml(rawData);
  ...
  handleValidation(xmlPath, xmlContent) {
    ...
    // then this document is too complex for libxmljs to validate (it only accepts one schema definition)
    ...
    this.writeToPanel('Document syntax check succeeded');
    ...
    if (schemaLocation && noNamespaceSchemaLocation) {
      this.writeToPanel('Document too complex for libxmljs, need a single schema on the root element to validate against. Unable to validate');
      return;
    ...
    this.fetchSchema(xmlPath, schemaLocation, ((err, schemaDoc) => {
      if (err) {
        this.writeToPanel(err);
      } else {
        this.performValidation(xmlDoc, schemaDoc);
      }
    }).bind(this));
  },
  ...
  performValidation(xmlDoc, schemaDoc) {
    try {
      let valid = xmlDoc.validate(schemaDoc);
      if (!valid) {
        _.each(xmlDoc.validationErrors, (valError) => {
          //this.writeToPanel('Validation error: ' + JSON.stringify(valError));
          this.writeToPanel('Validation error, line ' + valError.line + ': ' + valError);
        });
      } else {
  ...
  validateCurrent (editor, path) {
    ...
    if (path && path.endsWith('.xml')) {
      this.handleValidation(path, text);
    }
  },
